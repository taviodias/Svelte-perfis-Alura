<script lang="ts">
  import BarraSuperior from './BarraSuperior.svelte';
  import type IUsuario from '../interfaces/IUsuario'

  export let user: IUsuario|null;
  $: hasRepos = Boolean(user.last_repos.length);
</script>

<div class="card-usuario">
  <BarraSuperior />
  <div class="usuario">
    <div class="foto-container">
      <a href={user.html_url}>
        <div 
          class="foto-usuario"
          style:background-image="url({user.avatar_url})"
        ></div>
      </a>
    </div>
    <div class="detalhes-usuario">
      {#if user.name}
        <div class="info">
          Nome: <span>{user.name}</span>
        </div>
      {/if}
      <div class="info">
        Usuário: <span>{user.login}</span>
      </div>
      <div class="info">
        Seguidores: <span>{user.followers}</span>
      </div>
      <div class="info">
        Repositórios: <span>{user.public_repos}</span>
      </div>
    </div>
    {#if hasRepos}
    <div class="repositorios">
      <h2 class="titulo">Repositórios Recentes:</h2>
      <ul>
        {#each user.last_repos as repo}
          <li>
            <a href={repo.url} target="_blank" rel="noopener" class="repositorio">
              {repo.name}
            </a>
          </li>
        {/each}
      </ul>
    </div>
    {/if}
  </div>
</div>

<style>
  .card-usuario {
    margin-top: 65px;
  }

  .usuario {
    padding: 28px 0;
    background: rgba(255, 255, 255, 0.5);
    box-shadow: -12px 37px 45px rgba(133, 127, 201, 0.18);
    border-radius: 0px 0px 13px 13px;

    display: flex;
    justify-content: center;
  }

  .foto-container {
    margin-right: 81px;
  }

  .foto-usuario {
    width: 12.75rem;
    height: 12.75rem;
    border: 4.56px solid #2e80fa;
    border-radius: 50%;
    background-size: cover;
  }

  .detalhes-usuario {
    margin-right: 55px;
  }

  .detalhes-usuario > .info {
    font-weight: 600;
    font-size: 20px;
    line-height: 31px;
    color: #395278;
  }

  .detalhes-usuario > .info > span {
    color: #6781a8;
    font-weight: normal;
  }

  .repositorios > .titulo {
    font-size: 20px;
    line-height: 31px;
    font-weight: 600;
    color: #395278;
  }
  .repositorio {
    font-size: 20px;
    line-height: 31px;
    color: #6781a8;
    transition: color 0.2s;
  }
</style>